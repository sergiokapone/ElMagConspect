...
%=========================================================
\chapter[Рух заряджених частинок в електричних і\\ магнітних полях]{Рух заряджених частинок в електричних і магнітних полях}\label{\currfilebase}
...
Воно описує обертання навколо напрямку магнітного поля з кутовою швидкістю $\omega = qB/mc$, яка називається \emphx{циклотронною частотою}, --- рис.~\ref{tikz:cyclotron}.
...
\begin{equation}\label{eq:cyclotron_radius}
	R = \frac{v_{\perp}}{\omega} = \frac{mcv_{\perp}}{qB}.
\end{equation}
...
\end{equation}
де введена циклотронна частота $\omega = \sqrt{qB/mc}$. Виключаючи звідси змінну $V_x$, приведемо цю систему до рівняння гармонічного осцилятора:
...
\begin{figure}[h!]
	...
	\caption{Відхилення зарядженої частинки електричним полем конденсатора. К --- конденсатор, Е --- екран.}
...
\begin{equation}\label{eq:y_total_E}
	\tcbhighmath{
		y = y_1 + y_2 = \left(\frac{1}{2}l + L\right)\tg\alpha = \frac{qE}{mv_0}l\left(\frac{1}{2}l + L\right).
	}
\end{equation}
...
\begin{figure}[h!]
	...
	\caption{Відхилення зарядженої частинки магнітним полем. М --- магніт, Е --- екран. Напрямок магнітного поля перпендикулярно площині листа і напрямлено «за лист».}
...
Описаним методом Дж.~Дж.~Томсон в 1897~р. при вивченні катодних променів вперше виміряв питомий заряд електрона. Вимірювання показали, що питомий заряд електрона становить
\begin{equation}\label{eq:e_m_value}
	e/m = -5{,}273 \cdot 10^{17} \ \text{од.~СГСЕ} \cdot \text{г}^{-1}
	= -1{,}795 \cdot 10^{11} \ \text{Кл} \cdot \text{кг}^{-1}.
\end{equation}
...
Розглянемо метод докладніше. Будемо для визначеності вважати, що заряд краплі від'ємний, $q < 0$.
...
\begin{enumerate}
	...
	\begin{equation}\label{eq:gravity_force}
		\vect{F}_{\text{q}} = \frac{4\pi}{3}r^3(\rho - \rho_0)\vect{g},
	\end{equation}
...
За відсутності електричного поля крапля падає з постійною швидкістю $v_0$, яка визначається з умови $P_{\text{гр}} = F_{\text{гр}}$, або
