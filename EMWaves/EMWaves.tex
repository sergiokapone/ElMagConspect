...
\begin{figure}[h!]\centering
	\includegraphics[width=0.55\linewidth]{LebedevExp}
	\caption{Схема досліду П.~М.~Лебедєва (1899) з вимірювання тиску світла\label{pic:LebedevExp}}
\end{figure}

\subsection*{Практичне застосування тиску світла}

Тиск сонячного світла, попри свою мізерну величину (на орбіті Землі --- близько $5\cdot10^{-6}$~Па), знаходить практичне застосування в
\emphx{сонячних вітрилах} --- пристроях, що використовують тиск світла (сонячного чи лазерного) на дзеркальну поверхню для розгону космічного
апарата без витрати палива (рис.~\ref{pic:SolarSail}).
...
У межах проєкту \emphx{Breakthrough Starshot} пропонується розігнати мініатюрний зонд на лазерному вітрилі до швидкості $0.2c$ за допомогою
системи з десятків мільйонів лазерів, щоб досягти найближчої зорі --- Альфи Центавра --- за $\approx20$ років (замість $\approx100$ років
традиційними способами руху).

Тиск світла відіграє важливу роль і в природних явищах: в астрофізиці він, поряд із тиском газу, протидіє гравітаційному стисканню й забезпечує
стійкість зір (рис.~\ref{pic:StarComet}); саме тиском сонячного світла (разом із сонячним вітром) пояснюється форма кометних хвостів, завжди
спрямованих \emphx{від} Сонця, незалежно від напряму руху комети.
...
Використовуючи умову неперервності тангенціальної складової $\Hfield$ (з урахуванням зв'язку $B=nE/1$ та законів~\eqref{eq:SnellReflection}--\eqref{eq:SnellRefraction}), можна отримати амплітуди відбитої й заломленої хвиль через амплітуду падаючої --- \emphx{формули Френеля} для
$s$-поляризації:
...
Аналогічний розрахунок для $p$-поляризованої хвилі (рис.~\ref{pic:PPolarization}), виходячи з неперервності тангенціальних складових $\Hfield$
та $\Efield$, дає:
...
Якщо світло переходить з оптично густішого середовища в оптично менш густе, $n_2<n_1$, то, за законом заломлення, $\sin i/\sin r = n_2/n_1<1$,
тобто $i<r$: кут заломлення завжди \emphx{більший} за кут падіння (рис.~\ref{pic:TotalReflection}).

\begin{figure}[h!]\centering
	\includegraphics[width=0.4\linewidth]{totrefr}
	\caption{Заломлений промінь наближається до поверхні розділу при зростанні кута
		падіння\label{pic:TotalReflection}}
\end{figure}

Коли кут падіння сягає значення $i=i_b$, при якому кут заломлення $r=90^\circ$ (заломлений промінь ковзає вздовж самої межі розділу), кут $i_b$
називають \emphx{граничним кутом}:
...
\subsection*{Що відбувається за граничним кутом: загасаюча хвиля}

З'ясуємо, що відбувається з \enquote{заломленим променем} формально, за законами хвильової оптики, при $i>i_b$. Заломлена хвиля
$\Efield'' = \Efield''_m\, e^{i(\omega t-k''_xx-k''_yy)}$ повинна задовольняти співвідношення $k^2/n^2 = {k''_x}^2+{k''_y}^2$ (де $k=\omega/c$), а
з граничної умови $k_y=k''_y$ і $\sin i = k_y/k$ маємо
\begin{equation*}
	{k''_x}^2 = \frac{k^2}{n^2}\left(1-n^2\sin^2i\right) = \frac{\omega^2}{c^2}\left(1-n^2\sin^2i\right).
\end{equation*}
Оскільки при $i>i_b$ виконується $n^2\sin^2i>1$, вираз у дужках --- \emphx{від'ємний}, тому ${k''_x}^2<0$, тобто $k''_x$ --- \emphx{уявне} число:
$k''_x = iK$, де
\begin{equation*}
	K = \frac\omega c\sqrt{n^2\sin^2i-1} > 0.
\end{equation*}
...
\begin{Attention}
	Отже, поле в другому середовищі \emphx{існує} навіть при куті падіння, більшому за граничний, --- проте амплітуда цього поля \emphx{експоненційно
		спадає} вглиб другого середовища (уздовж $x$), а не поширюється як біжуча хвиля. Таке поле називають \emphx{загасаючою (evanescent) хвилею}:
	воно \enquote{чіпляється} за межу розділу й проникає в друге середовище на глибину порядку довжини хвилі світла $\lambda_0$, не переносячи
	при цьому енергії \emphx{в середньому} углиб середовища (потік енергії вздовж $x$, усереднений за період, дорівнює нулю --- саме тому
	відбивання й називають \emphx{повним}). Якщо ж на відстані порядку $\lambda_0$ від межі розділу розташувати третє середовище (наприклад,
	наблизити впритул ще один шматок скла до межі повного внутрішнього відбивання), \enquote{хвіст} загасаючої хвилі виявиться там ще достатньо
	відчутним, щоб розхитати електрони цього середовища й породити нову, вже \emphx{біжучу} хвилю --- частина світла \emphx{пройде} крізь
	проміжок, попри те, що кожна з двох меж окремо давала б повне відбивання. Це явище називають \emphx{порушеним повним внутрішнім відбиванням}
	(frustrated total internal reflection) --- оптичний аналог квантовомеханічного тунелювання, який легко спостерігати експериментально для
	сантиметрових радіохвиль (де $\lambda_0$ --- сантиметри, тому проміжок легко зробити порівнянним з довжиною хвилі) і значно важче --- для
	видимого світла (де $\lambda_0\sim10^{-5}$~см вимагає надзвичайно вузького проміжку).
\end{Attention}
