...
%=========================================================
\epigraph{\Alexander  Не проводячи струм, вони проводять ідеї про взаємодію матерії та поля.
...
Речовина, внесена в електричне поле, може істотно змінити його. Це пов'язано з тим, що речовина складається із заряджених частинок. За відсутності
зовнішнього поля частинки розподіляються всередині речовини так, що електричне поле, яке вони створюють, у середньому за об'ємом, що включають велику
кількість атомів або молекул, дорівнює нулю. За наявності зовнішнього поля відбувається перерозподіл заряджених частинок, і в речовині виникає власне
електричне поле. Повне електричне поле складається відповідно до принципу суперпозиції із зовнішнього поля і внутрішнього поля, що створюється
зарядженими частинками речовини. Як ми бачили, в металах внутрішнє поле повністю компенсувало зовнішнє поле, тому всередині металів результуюче поле
дорівнювало нулю.


На відміну від провідників, у діелектриках немає вільних електричних зарядів, оскільки вони складаються з нейтральних атомів або молекул. Заряджені
частинки в нейтральному атомі зв'язані одна з одною і не можуть переміщатися під дією електричного поля по всьому об'єму діелектрика. Зробимо умовну
класифікацію зарядів:
\begin{enumerate}
    \item \emphx{Вільні заряди} --- це заряди, які можуть переміщатися на великі відстані в речовині
    (набагато більші за міжатомні відстані). У діелектриках вільних зарядів, як правило, мало.
    \item \emphx{Зв'язані (поляризаційні) заряди} --- це заряди, які під дією зовнішніх полів або сил мало
    зміщуються відносно свого положення рівноваги і повертаються назад, у положення рівноваги, після
    зняття зовнішнього впливу.
\end{enumerate}


При внесенні діелектрика в зовнішнє електричне поле в ньому виникає перерозподіл зв'язаних зарядів таким чином, що виникає об'ємний дипольний момент,
який створює електричне поле, спрямоване протилежно зовнішньому полю. Цей процес називається \emphx{поляризацією} діелектрика. У результаті повне
електричне поле всередині діелектрика виявляється за модулем меншим за зовнішнє поле.
Однак, поляризація може виникати як не лише завдяки зовнішньому електричному полю, а і під впливом інших зовнішніх чинників, наприклад механічного
впливу, чи температури. Поляризація не змінює сумарний заряду речовини, а лише впливає на перерозподіл заряду в речовині.


%% --------------------------------------------------------
\subsection*{Мікроскопічні поля}
%% --------------------------------------------------------

При мікроскопічному погляді на речовину, зрозуміло, що електричне поле різко змінюється від точки до точки. В середині атома чи молекули воно набагато
сильніше ніж в міжмолекулярному просторі. Таке поле називається \emphx{мікрополем} $\Efield_\text{micro}$. Але ж всі вимірювання ми проводимо
макроскопічними приладами, а тому можемо визначити не мікрополе, а усереднене поле по деякому фізично нескінченно малому об'єму $\Delta V$:
\begin{equation*}
	\left\langle \Efield\right\rangle = \frac1{\Delta V} \iiint\limits_{\Delta V}
	\Efield_\text{micro} dV.
\end{equation*}
Це поле змінюється істотно повільніше ніж мікрополе. Коли ми говоримо про макроскопічні величини, що характеризують діелектрик, ми завжди маємо на увазі
саме усереднені вказаним способом величини.
...
Всі діелектрики можна розділити на два основних класа:
\begin{enumerate}
    \item \emphx{Полярні діелектрики} --- складаються з полярних молекул, що мають дипольний момент.
    \item \emphx{Неполярні діелектрики} --- складаються з неполярних молекул, у яких центри позитивного і негативного зарядів збігаються.
\end{enumerate}
...
\emphx{Вектор поляризації (поляризованість)} --- це дипольний момент одиниці об'єму
речовини (густина
дипольного моменту):
$
...
%		Вміщення діелектрика в електричне поле також призводить до \emphx{зміни розмірів тіл}. Цей
%		ефект називається  \emphx{електрострикцією}. У всіх діелектриках спостерігається
%		електрострикція.
%	\end{block}
%
%	\begin{block}{}\justifying
...
%
%	\begin{block}{}\justifying
%		Для великого класу діелектриків \emphx{поляризованість залежить лінійно
...
%			}
%		\end{equation*}
%		де $\chi > 0$ --- безрозмірна величина називається \emphx{діелектричною сприйнятливістю
...
%	\begin{block}{}\justifying
%		Поляризація називається \emphx{однорідною}, якщо вектор поляризації $\vect{P}$ є
%		постійним за об'ємом речовини: $\vect{P} = \const$, і \emphx{неоднорідною}, якщо
%		$\vect{P}$ змінюється від точки до точки.
%	\end{block}
%	\begin{onlyenv}<1>
%		\begin{block}{}\justifying
%			Нехай поляризація \emphx{однорідна}. Розглянемо косокутний паралелепіпед, вирізаний із
...
%	\begin{onlyenv}<2-4>
%		\begin{block}{}\justifying
%			Нехай тепер поляризація \emphx{неоднорідна}. Розглянемо в речовині деякий об'єм
%			довільної форми.
%		\end{block}
%		\begin{columns}
...
%							}
%						\end{equation*}
%						Цей вираз складає \emphx{теорему Гаусса для вектора поляризації}.
%					\end{block}
%					\onslide<4>
%					\begin{block}{}
...
%	\begin{onlyenv}<1-2>
%		\begin{block}{}
%			Введений у вектор $\Dfield$ називається \emphx{вектором електричної індукції}.
%		\end{block}
%	\end{onlyenv}
%	\begin{onlyenv}<2>
%		\begin{block}{}
...
%		\end{columns}
%		\begin{block}{}\justifying
%			У формулювання теореми Гаусса для поля в речовині \emphx{входять тільки вільні заряди}.
...
%			}
%		$
%		називається \emphx{діелектричною проникністю середовища}. Для випадку постійних електричних полів
...
%				\emphx{поляризаційні заряди призводять до ослаблення поля}.
%			\end{block}
%		\end{column}
%	\end{columns}
%\end{frame}
...
%			\end{equation*}
%			обернено пропорційна температурі $\chi \sim \frac1T$. Ця залежність виду називається \emphx{законом
...
%			зовнішнього поля. Електрет є <<\emphx{електричним аналогом постійного магніту}>>.
%		\end{block}
%		\tikzset{
%			cylinder/.pic={
%					\begin{scope}
...
%			затверділій речовині ускладнений. У результаті отримують \emphx{<<заморожену>>
%				поляризацію}.
%		\end{block}
%
%		\begin{block}{}\justifying\small
...
%	\begin{block}{}\justifying
%		\emphx{Сегнетоелектрик} --- це кристалічний діелектрик, який має \emphx{спонтанну поляризацію
...
%			\begin{block}{}\justifying
%				\emphx{Домен} --- це локальна область усередині сегнетоелектрика, де дипольні моменти
...
%	\begin{block}{}\justifying
%		\emphx{Гістерезис} --- неоднозначна петлеподібна залежність поляризації сегнетоелектриків від
...
%		      яке називається \emphx{коерцитивною силою}.
...
%	\begin{block}{}\justifying
%		\emphx{П'єзоелектрики} --- діелектрики, що можуть або під дією деформації індукувати
...
%
%	\begin{block}{}\justifying
%		\emphx{Піроелектрики} --- кристалічні діелектрики, що мають спонтанну поляризацію, тобто
