...
\begin{figure}[h!]
	...
	\begin{circuitikz}[european resistors, scale=1.5, circuitikz/bipoles/length=2.1cm, thick]
		% --- 1. КЛЮЧОВІ КООРДИНАТИ (змінюйте їх для масштабування усієї схеми) ---
		\coordinate (A) at (0,0); % Лівий нижній кут
		\coordinate (B) at (4,0); % Правий нижній кут
		\coordinate (C) at (4,3); % Правий верхній кут
		\coordinate (D) at (0,3); % Лівий верхній кут

		% Клеми на нижній гілці (відносне розміщення)
		\coordinate (T1) at ($(A)!0.4!(B)$); % 30% відстані від A до B
		\coordinate (T2) at ($(A)!0.6!(B)$); % 70% відстані від A до B

		% --- 2. ЕЛЕМЕНТИ СХЕМИ ---
		% Нижня гілка з клемами
		...
		% Верхня гілка: резистор
		\draw[fill=green!50!black] (C) to[R, l_=$R$, color=green!50!black] (D);

		% Стрілка струму J (прив'язана до відрізка CD)
		\draw[->, >=latex] ($(C)!0.325!(D) + (0,-0.5)$) -- ($(C)!0.675!(D) + (0,-0.5)$)
		node[midway, below] {$J$};

		% Ліва гілка: конденсатор
		\draw (D) to[C, l_=$C$, color=red] (A);

		% --- 3. ПІДПИСИ (динамічно розраховуються відносно відрізка DA) ---
...
\begin{Attention}
	Історія цієї формули --- це історія відкриття, яке на кілька десятиліть випередило теорію, покликану його пояснювати. Ще в 1827 році французький фізик Фелікс Саварі, розряджаючи лейденську банку через дріт, за характером перемагнічування дрібних сталевих голок здогадався, що розряд не монотонний, а коливальний. У 1842 році американець Джозеф Генрі підтвердив цю догадку і навіть показав, що іскра здатна намагнічувати голку на відстані 30 футів крізь стіни й підлоги --- хоча пояснити це поширенням електромагнітних хвиль тоді не міг ніхто: самого поняття електромагнітної хвилі ще не існувало. Строгий математичний опис коливального розряду конденсатора через котушку дав лише в 1853 році Вільям Томсон (майбутній лорд Кельвін) --- звідси й назва формули. А побачити криву затухаючих коливань на власні очі вдалося німецькому фізику Беренду Феддерсену: у 1857--1862 роках він фотографував іскровий розряд на швидко обертовому дзеркалі, отримуючи щось на зразок першої в світі швидкісної фотозйомки електричного процесу. Замкнути це коло довелося вже Генріху Герцу, який у 1888 році експериментально довів існування електромагнітних хвиль, передбачених рівняннями Максвела.
\end{Attention}
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}
		\begin{axis}[
				% Вісі зі стрілками по центрах/краях
				axis lines=center,
				xlabel={$t$},
				ylabel={$q$},
			...
			% Верхня огинаюча (пунктир): q0 * exp(-gamma * t)
			\addplot [dashed, domain=0:9.5, thick] {exp(-0.35*x)};

			% Нижня огинаюча (пунктир): -q0 * exp(-gamma * t)
			\addplot [dashed, domain=0:9.5, thick] {-exp(-0.35*x)};

			% Основний графік згасаючих коливань: q0 * exp(-gamma * t) * cos(omega * t)
			\addplot [red, thick, domain=0:9.5] {exp(-0.35*x) * cos(deg(2.5*pi*x))};

			% Вказівка та підпис для верхньої огинаючої
			\node (top_label) at (axis cs:7.5, 0.75) {$q_0\exp(-\gamma t)$};
			\draw[->, >=stealth, thick] (top_label.west) -- (axis cs:5.2, 0.22);

			% Вказівка та підпис для нижньої огинаючої
...
Запишемо рівняння \eqref{eq:circuit_equation} для $LCR$-контуру за наявності ЕРС $\emf$,
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% Позначення V_ст на осі Y
			\node[left] at (axis cs:0, 0.6) {$V_{\text{ст}}$};

			% Резонансна частота omega_0 (вертикальна штрихова лінія)
			\draw[dashed, thick] (axis cs:1.0, 0) -- (axis cs:1.0, 2.3);
			\node[below=2pt] at (axis cs:1.0, 0) {$\omega_0$};

			% 1. gamma_1 = 0 (Нескінченний резонанс, розрив при omega = omega_0)
			...
			% 2. gamma_2 (Мале згасання, високий пік)
			% Формула: V_0 = V_ст / sqrt((1 - x^2)^2 + 4 * gamma^2 * x^2)
			\addplot [red, thick, domain=0:2.3] {0.6 / sqrt((1 - x^2)^2 + 4*(0.18^2)*x^2)};
			\node at (axis cs:0.88, 1.35) {$\gamma_2$};

			% 3. gamma_3 (Середнє згасання, низький зсунутий пік)
			\addplot [red, thick, domain=0:2.3] {0.6 / sqrt((1 - x^2)^2 + 4*(0.45^2)*x^2)};
			\node at (axis cs:0.78, 0.5) {$\gamma_3$};

			% 4. gamma_4 > omega_0 / sqrt(2) (Аперіодичний режим / без резонансного піку)
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}
		\begin{axis}[
				...
				xmin=0, xmax=2.5,
				ymin=-3.5, ymax=0.5,
				% Без автоматичних чисел/засічок
				ticks=none,
				% Розміри
				width=10cm,
				height=7cm,
			...
			% Формула фазового зсуву: \varphi_0(\omega) = -atan2(2*\gamma*\omega, \omega_0^2 - \omega^2)
			% У TikZ/PGFPlots atan2(y, x) повертає кут у градусах, тому переводимо в радіани (* pi / 180)

			% 1. gamma_1 (дуже мале згасання, різкий стрибок)
			...
			% 4. gamma_4 (велике згасання, плавна крива)
			\addplot [red, thick, domain=0.01:2.3] {-atan2(2*0.8*x, 1 - x^2) * pi / 180};

			% Позначення кривих gamma_1, gamma_2, gamma_3, gamma_4 з виносними лініями
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}
		\begin{axis}[
				% Параметри та математичні функції
				declare function={
						w0 = 1.0;                  % Власна частота
						gamma = 0.15;              % Коефіцієнт згасання
						Vst = 0.6;                 % Статична амплітуда
						% Резонансна крива
						V0(\w) = Vst / sqrt((w0^2 - \w^2)^2 + 4*(gamma^2)*(\w^2));
						% Точні значення для побудови
						w_res = sqrt(w0^2 - 2*gamma^2);                    % Частота максимуму
						Vmax = V0(w_res);                                  % Амплітуда максимуму
						Vlevel = Vmax / sqrt(2);                           % Рівень V_max / sqrt(2)
						% Точки перетину з рівнем Vmax / sqrt(2)
						w1 = sqrt(w0^2 - 2*gamma^2 + 2*gamma*w_res);       % Ліва точка
						w2 = sqrt(w0^2 - 2*gamma^2 - 2*gamma*w_res);       % Правильніше за формулою
					},
				% Налаштування осей
				axis lines=center,
				xlabel={$\omega$},
			...
			% Розрахунок точок перетину w1, w2 для стрілок через розв'язок рівняння V0(w) = Vlevel
			% w^2 = (w0^2 - 2*gamma^2) +- 2*gamma*sqrt(w0^2 - gamma^2)
			...
			% --- Засічка та підпис на осі X ---
			\draw[thick] (axis cs:{w0}, -0.04) -- (axis cs:{w0}, 0.04) node[below=3pt] {$\omega_0$};

			% --- Резонансна крива ---
			\addplot [red, thick, domain=0:2.3] {V0(x)};

			% --- Динамічні стрілки ширини резонансу Delta w ---
			% Втикаються точно в обчислені точки перетину (\wone та \wtwo)
...
\begin{Attention}
	...
	Явище резонансу напруг використовують в \emph{електричних фільтрах}: наприклад, якщо необхідно усунути з сигналу, що передається, складову струму певної частоти, то паралельно приймачу ставлять ланцюжок із сполучених послідовно конденсатора і котушки індуктивності, щоб струм резонансної частоти цього $LC$-ланцюжка замикався через нього і не потрапляв до приймача. Тоді струми з частотою, далекою від резонансної частоти $LC$-ланцюжка, проходитимуть у навантаження безперешкодно, і тільки близькі до резонансу за частотою струми знаходитимуть собі найкоротший шлях через $LC$-ланцюжок.

	Резонанс напруг широко застосовують у радіотехніці --- у тих випадках, коли потрібно посилити коливання напруги певної визначеної частоти, при настроюванні радіоприймача на потрібну довжину хвилі. Вибірність приймача буде тим більшою, чим «гостріша» резонансна крива струму. Але уявлення про те, що під час радіоприймання потрібно, по можливості, збільшувати добротність контуру --- бо при цьому зростає напруга на конденсаторі, тобто чутливість приймача, і усувається вплив сусідніх станцій, оскільки резонансна крива звужується --- є помилковим, оскільки сигнал ніколи не буває ідеально монохроматичним. Тому, якщо крива резонансу надзвичайно вузька, частина спектральних складових сигналу опиниться поза смугою пропускання контуру, що призведе до викривлення сигналу. Саме тому, наприклад, у телебаченні при передачі сигналів зображення використовують широку смугу частот.
\end{Attention}
...
%	\ddot{V} + \omega_0^2 V = \omega_0^2 \emf_0 \cos\omega t.
%\end{equation}
...
%	+ \underbrace{\frac{\omega_0^2 \emf_0}{\omega_0^2 - \omega^2} \cos\omega t}_{V_f(t)}.
%\end{equation}
%
%\begin{figure}[h!]
%	\centering
%	\begin{circuitikz}[scale=1.5, circuitikz/bipoles/length=2.1cm, thick]
%		% --- 1. КЛЮЧОВІ КООРДИНАТИ ---
%		\coordinate (A) at (0,0); % Лівий нижній кут
%		\coordinate (B) at (4,0); % Правий нижній кут
%		\coordinate (C) at (4,3); % Правий верхній кут
%		\coordinate (D) at (0,3); % Лівий верхній кут
%
%		% Точки для розміщення елементів на нижній гілці
%		\coordinate (T1) at ($(A)!0.3!(B)$); % Ліва клема
%		\coordinate (T2) at ($(A)!0.5!(B)$); % Права клема джерела
%		\coordinate (T3) at ($(A)!0.85!(B)$); % Клема ключа K
%
%		% --- 2. ЕЛЕМЕНТИ СХЕМИ ---
%
%		% Нижня гілка: Джерело (через клеми 1 i 2) та Ключ K (switch)
%		\draw (A) to[short, -o] (T1);
%		\draw (T2) to[nos, o-o, l_=$K$] (T3); % Ключ K (normal open switch)
%		\draw (T3) -- (B);
%
%		% Підпис джерела ЕРС між клемами
%		\path (T1) -- (T2) node[midway, below=2pt] {$\mathcal{E}$};
%
%		% Права гілка: котушка індуктивності L
%		\draw (B) to[L, mirror, l_=$L$] (C);
%
%		% Верхня гілка: просто провідник
%		\draw (C) -- (D);
%
%		% Стрілка струму J (прив'язана до відрізка CD)
%		\draw[->, >=latex] ($(C)!0.35!(D) + (0,-0.4)$) -- ($(C)!0.65!(D) + (0,-0.4)$)
%		node[midway, below] {$J$};
%
%		% Ліва гілка: конденсатор C
%		\draw (D) to[C, l_=$C$] (A);
%
%		% --- 3. ПІДПИСИ ЗАРЯДІВ НА ОБКЛАДКАХ ---
%		\node at ($(D)!0.32!(A) + (-0.4,0)$) {$+q$};
%		\node at ($(D)!0.68!(A) + (-0.4,0)$) {$-q$};
%	\end{circuitikz}
...
%	V(0) = 0 \ \Rightarrow\ a + \emf_0 \frac{\omega_0^2}{\omega_0^2 - \omega^2} = 0
%	\ \Rightarrow\ a = -\emf_0 \frac{\omega_0^2}{\omega_0^2 - \omega^2},
%\end{equation}
...
%	V(t) = \emf_0 \frac{\omega_0^2}{\omega_0^2 - \omega^2}\left(\cos\omega t - \cos\omega_0 t\right).
%\end{equation}
...
%	\tcbhighmath{
%		V(t) = \emf_0 \frac{2\omega_0^2}{\omega^2 - \omega_0^2}\sin\left(\frac{\Delta\omega}{2}t\right)\sin(\bar{\omega}t).
%	}
%\end{equation}
%
...
%	A(t) = \emf_0 \frac{2\omega_0^2}{\omega^2 - \omega_0^2}\sin\left(\frac{\Delta\omega}{2}t\right)
%\end{equation}
...
%        tNode2 = 2*TA;
%        tZeroStart = 1.25*TA;
%        tZeroEnd = tZeroStart + T0;
%        env(\t) = abs(sin(deg(dw*\t/2)));
%        V(\t) = sin(deg(dw*\t/2)) * sin(deg(w*\t));
%    },
%    axis lines=center,
%    xlabel={$t$},
%    ylabel={$V$},
%    x label style={anchor=south west, at={(axis description cs:0.95,0.52)}},
%    y label style={anchor=south east, at={(axis description cs:0.02,0.95)}},
%    xmin=0, xmax={2.15*TA},
%    ymin=-1.4, ymax=1.4,
%    xtick=\empty,
%    ytick=\empty,
...
%]
%
%    % --- 1. ОГИНАЮЧІ ЛІНІЇ (пунктир) ---
%    \addplot [dashed, thick, domain=0:{2.05*TA}] {env(x)};
%    \addplot [dashed, thick, domain=0:{2.05*TA}] {-env(x)};
%
%    % --- 2. ШВИДКІ КОЛИВАННЯ (биття) ---
%    \addplot [red, thick, domain=0:{2.05*TA}] {V(x)};
%
%    % --- 3. ШТРИХОВІ ЛІНІЇ ВУЗЛІВ ТА ПЕРІОДІВ ---
%    \draw[dashed, thick] (axis cs:{tNode1}, -1.2) -- (axis cs:{tNode1}, 0);
%    \draw[dashed, thick] (axis cs:{tNode2}, -1.2) -- (axis cs:{tNode2}, 0);
%    \draw[dashed, thick] (axis cs:{tZeroStart}, -0.9) -- (axis cs:{tZeroStart}, {V(tZeroStart)});
%    \draw[dashed, thick] (axis cs:{tZeroEnd}, -0.9) -- (axis cs:{tZeroEnd}, {V(tZeroEnd)});
%
%    % --- 4. ДВОСТОРОННІ СТРІЛКИ ТА ПІДПИСИ ПЕРІОДІВ ---
%    \draw[<->, >=stealth, thick] (axis cs:0, -0.9) -- (axis cs:{tNode1}, -0.9)
%        node[midway, below=3pt] {$T_A = 2\pi/\Delta\omega$};
%    \draw[<->, >=stealth, thick] (axis cs:{tZeroStart}, -0.8) -- (axis cs:{tZeroEnd}, -0.8)
%        node[midway, below=3pt] {$T_0 = 2\pi/\omega$};
%
%\end{axis}
%\end{tikzpicture}
...
%з часом затухнуть: $V_s(t) \xrightarrow{t\to\infty} 0$, і в контурі встановляться гармонічні вимушені коливання $V_f(t)$.
...
\begin{equation}\label{eq:complex_Ohm_equation}
	L\ddot{q} + R\dot{q} + \frac{q}{C} = \emf(t), \qquad
	\emf(t) = \emf_0 e^{i\omega t}.
\end{equation}
...
що $\dot{\emf}(t) = i\omega \emf_0 e^{i\omega t} = i\omega \emf(t)$:
\begin{equation}\label{eq:current_equation}
	L\dot{J} + RJ + \frac{J}{C} = i\omega \emf_0 e^{i\omega t}.
\end{equation}
...
\begin{equation}\label{eq:energy_balance_LCR}
	\frac{dW}{dt} = -RJ^2 + J\emf, \qquad
	W = \frac{LJ^2}{2} + \frac{q^2}{2C}.
\end{equation}
Тут у правій частині рівняння враховані постійне надходження енергії від зовнішньої ЕРС ($J\emf$), що виконує роботу над струмами за законом Джоуля---Ленца, а також втрати енергії в активному опорі ($-RJ^2$).
...
\begin{equation}\label{eq:Ohm_vector}
	RJ + i\omega L J - i\frac{1}{\omega C}J = \emf.
\end{equation}
...
Сума трьох векторів у лівій частині рівняння \eqref{eq:Ohm_vector} дає вектор $\emf$. З діаграми видно, що довжина цього вектора (амплітуда коливань ЕРС) пов'язана з довжиною вектора струму (амплітудою коливань струму) співвідношенням
...
\begin{equation}\label{eq:current_amplitude}
	\tcbhighmath{
		J_0 = \frac{\emf_0}{\sqrt{R^2 + \left(\omega L - \frac{1}{\omega C}\right)^2}}.
	}
\end{equation}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[>=Stealth, line width=0.8pt]
        ...
        % ==========================================
        % ЛІВА ЧАСТИНА: Напрямок обертання e^(i*omega*t)
        % ==========================================
        \begin{scope}[shift={(0,0)}]
            ...
            % Вектор e^(i*omega*t)
            \draw[red, ->, thick] (O1) -- (30:2.8) node[right, text=black] {$e^{i\omega t}$};

            % Дуга напрямку обертання
            \draw[->, thick] (10:2.0) arc (10:70:2.0);

            % Підпис напрямку обертання
            \node[align=center, font=\scriptsize] at (1.8, 2.7) {Позитивний\\напрямок\\обертання};
            \draw[->] (1.8, 2.1) -- (1.4, 1.6);
        \end{scope}

        % ==========================================
        % ПРАВА ЧАСТИНА: Векторна діаграма (абсолютно динамічні координати)
        % ==========================================
        \begin{scope}[shift={(6.5,0)}]
            % --- 1. ДИНАМІЧНІ ПАРАМЕТРИ ТА РОЗРАХУНКИ ---
            \pgfmathsetmacro{\uR}{4.0}                             % Амплітуда активної напруги (RJ)
            \pgfmathsetmacro{\uL}{3.2}                             % Амплітуда індуктивної напруги (wLJ)
            \pgfmathsetmacro{\uC}{1.1}                             % Амплітуда ємнісної напруги (J/wC)
            \pgfmathsetmacro{\uReac}{\uL - \uC}                    % Результуюча реактивна напруга
            \pgfmathsetmacro{\phiDeg}{atan2(\uReac, \uR)}          % Динамічний кут фази phi_0 в градусах

            % --- 2. ДИНАМІЧНЕ ОГОЛОШЕННЯ ТОЧОК ---
            \coordinate (O2)     at (0, 0);
            \coordinate (A)      at (\uR, 0);                      % Кінець вектора RJ
            \coordinate (UL_OY)  at (0, \uL);                      % Вектор індуктивності на OY
            \coordinate (UC_OY)  at (0, -\uC);                     % Вектор ємності на OY
            \coordinate (C)      at (\uR, \uReac);                 % Вершина сумарного вектора ЕРС
            \coordinate (U_reac) at (0, \uReac);                   % Проекція ЕРС на вісь OY
...
thick]
		...
		% --- 1. ВЕРШИНИ ТА КОРДИНАТИ ---
		% Ліва та права вертикальні шини
		\coordinate (L0) at (0, 0);   % Нижня ліва точка (джерело)
		\coordinate (L1) at (0, 1.2); % Ліва точка резистора R
		\coordinate (L2) at (0, 2.4); % Ліва точка конденсатора C
		\coordinate (L3) at (0, 3.6); % Ліва точка котушки L

		\coordinate (R0) at (3.5, 0);   % Нижня права точка
		\coordinate (R1) at (3.5, 1.2); % Права точка резистора R
		\coordinate (R2) at (3.5, 2.4); % Права точка конденсатора C
		\coordinate (R3) at (3.5, 3.6); % Права точка котушки L

		% --- 2. ЕЛЕМЕНТИ СХЕМИ ---

		% Верхня гілка: Котушка індуктивності L
		\draw (L3) to[L, l=$L$] (R3);
		% Стрілка струму J_L (вліво)
		\draw[<-, >=latex, thick] ($(L3)!0.82!(R3) + (0, 0.35)$) -- ($(L3)!0.98!(R3) + (0, 0.35)$)
		node[midway, above] {$J_L$};

		% Середня верхня гілка: Конденсатор C
		\draw (L2) to[C, l=$C$] (R2);
		% Стрілка струму J_C (вліво)
		\draw[<-, >=latex, thick] ($(L2)!0.82!(R2) + (0, 0.35)$) -- ($(L2)!0.98!(R2) + (0, 0.35)$)
		node[midway, above] {$J_C$};

		% Середня нижня гілка: Резистор R
		\draw (L1) to[R, l=$R$] (R1);
		% Стрілка струму J_R (вліво)
		\draw[<-, >=latex, thick] ($(L1)!0.82!(R1) + (0, 0.35)$) -- ($(L1)!0.98!(R1) + (0, 0.35)$)
		node[midway, above] {$J_R$};

		% Нижня гілка: Джерело струму J(t)
		\draw (L0) to[isourcesin, sources/symbol/rotate=0, l_=$J(t)$] (R0);

		% --- 3. З'ЄДНУВАЛЬНІ ПРОВІДНИКИ ---
		\draw (L0) -- (L3); % Ліва шина
		\draw (R0) -- (R3); % Права шина

		% --- 4. ЗАГАЛЬНИЙ СТРУМ J ВПРОВОДУ (вгору) ---
...
\begin{Attention}
	...
	В індукційних печах паралельно котушці, що створює змінне магнітне поле, підключають ємність. Якщо умови резонансу виконуються, то струм в котушці збільшується в багато разів, а струм в підвідних проводах при цьому відносно невеликий. Той самий принцип --- лише в мініатюрі --- лежить в основі сучасних побутових індукційних варильних поверхонь і безконтактної зарядки телефонів: замість тонни розплавленого металу тут вистачає котушки під склокерамічною плитою чи в зарядній панелі.
\end{Attention}
	...
	\item Для будь-якого замкнутого контуру в квазістаціонарному електричному колі виконується рівність
	      ...
	      \begin{figure}[h!]
		      ...
		      \begin{tikzpicture}[>=Stealth, line width=1.2pt]
			      ...
			      % --- ВІТКИ ЗІ СТРІЛКАМИ ДО ЦЕНТРУ ---

			      % Ліва верхня гілка (J1)
			      \draw[->] (-1.5, 1.3) -- (O) node[above left=1pt, pos=0] {$J_1$};

			      % Права верхня гілка (J2)
			      \draw[->] (1.5, 1.3) -- (O) node[above right=1pt, pos=0] {$J_2$};

			      % Нижня вертикальна гілка (J3)
			      \draw[<-] (0, -2.0) -- (O) node[midway, right=3pt] {$J_3$};

			      % Точка вузла в центрі
			      \fill (O) circle (2pt);
...
\begin{enumerate}
	\item Паралельно з'єднані імпеданси (рис.~\ref{tikz:parallel_impedances}).
	      ...
	      \begin{figure}[h!]
		      ...
		      \begin{circuitikz}[european resistors, scale=1.4, circuitikz/bipoles/length=1.8cm, thick]
			      ...
			      % --- 1. ОСНОВНІ КООРДИНАТИ ---
			      \coordinate (A) at (0,0);   % Нижня ліва точка
			      \coordinate (B) at (2,0);   % Нижня середня точка
			      \coordinate (C) at (4.5,0); % Нижня права точка

			      \coordinate (D) at (0,3);   % Верхня ліва точка
			      \coordinate (E) at (2,3);   % Верхня середня точка (вузол)
			      \coordinate (F) at (4.5,3); % Верхня права точка

			      % Клеми джерела змінного струму
			      \coordinate (T1) at ($(C)!0.3!(F)$);
			      \coordinate (T2) at ($(C)!0.7!(F)$);

			      % --- 2. ЕЛЕМЕНТИ СХЕМИ ---

			      % Ліва гілка: Імпеданс Z1
			      \draw (D) to[R, l_=$Z_1$] (A);

			      % Середня гілка: Імпеданс Z2
			      \draw (E) to[R, l_=$Z_2$] (B);

			      % Права гілка: Клеми джерела ЕРС ~E
			      \draw (C) to[short, -o] (T1);
			      \draw (F) to[short, -o] (T2);
			      \node[right=2pt] at ($(T1)!0.5!(T2)$) {$\sim\!\emf$};

			      % Провідники верхньої та нижньої шин
			      \draw (A) -- (C);
			      \draw (D) -- (F);

			      % --- 3. СТРІЛКИ СТРУМІВ ---

			      % Струм J1 (напрямлений вліво до Z1)
			      \draw[->, >=latex, thick] ($(E)!0.3!(D) + (0, 0.3)$) -- ($(E)!0.7!(D) + (0, 0.3)$)
			      node[midway, above=1pt] {$J_1$};

			      % Струм J2 (напрямлений вниз до Z2)
			      \draw[->, >=latex, thick] ($(E)!0.1!(B) + (0.35, 0)$) -- ($(E)!0.45!(B) + (0.35, 0)$)
			      node[midway, right=1pt] {$J_2$};

			      % Загальний струм J (напрямлений вліво від джерела)
			      \draw[->, >=latex, thick] ($(F)!0.3!(E) + (0, 0.3)$) -- ($(F)!0.7!(E) + (0, 0.3)$)
			      node[midway, above=1pt] {$J$};
	      ...
	      \begin{equation}\label{eq:loop_equations}
		      J_1 Z_1 = \emf \quad\text{--- для контуру } (Z_1, \emf),
	      \end{equation}
	      \begin{equation}\label{eq:loop_equations_2}
		      J_2 Z_2 = \emf \quad\text{--- для контуру } (Z_2, \emf).
	      \end{equation}
	      ...
	      \begin{equation}\label{eq:current_sum_expanded}
		      J = J_1 + J_2, \quad\text{або}\quad \frac{\emf}{Z} = \frac{\emf}{Z_1} + \frac{\emf}{Z_2}.
	      \end{equation}
	...
	\item  Послідовно з'єднані імпеданси (рис.~\ref{tikz:series_impedances}).
	      ...
	      \begin{figure}[h!]
		      ...
		      \begin{circuitikz}[european resistors, scale=1.4, circuitikz/bipoles/length=1.8cm, thick]
			      ...
			      % --- 1. КОРДИНАТИ СХЕМИ ---
			      \coordinate (A) at (0,0);   % Лівий нижній кут
			      \coordinate (B) at (3.5,0); % Правий нижній кут (клема)
			      \coordinate (C) at (3.5,2); % Правий верхній кут (клема)
			      \coordinate (D) at (0,2);   % Лівий верхній кут

			      % Клеми джерела змінного струму
			      \coordinate (T1) at ($(B)!0.2!(C)$);
			      \coordinate (T2) at ($(B)!0.8!(C)$);

			      % --- 2. ЕЛЕМЕНТИ СХЕМИ ---

			      % Верхня гілка: Імпеданс Z1
			      \draw (C) to[R, l_=$Z_1$] (D);

			      % Нижня гілка: Імпеданс Z2
			      \draw (A) to[R, l_=$Z_2$] (B);

			      % Ліва гілка: Провідник зі стрілкою струму J (вниз)
			      \draw (D) -- (A);
			      \draw[->, >=latex, thick] ($(D)!0.3!(A) + (0.35,0)$) -- ($(D)!0.7!(A) + (0.35,0)$)
			      node[midway, right=1pt] {$J$};

			      % Права гілка: Клеми джерела ЕРС ~E
			      \draw (B) to[short, -o] (T1);
			      \draw (C) to[short, -o] (T2);
			      \node[right=2pt] at ($(T1)!0.5!(T2)$) {$\sim\!\mathcal{E}$};
...
\begin{Attention}
	Формула $\overline{Q} = \emf_{\text{eff}} J_{\text{eff}} \cos\varphi_0$ --- це не просто гарний результат для підручника, а цілком реальний рядок у рахунку за електроенергію. Промислові споживачі зі значним індуктивним навантаженням --- потужні електродвигуни, трансформатори, зварювальні апарати --- забирають з мережі великий струм навіть тоді, коли корисна (активна) потужність невелика, просто через низький $\cos\varphi_0$. Провідники, трансформатори і генератори при цьому однаково перевантажуються реактивним струмом, який жодної корисної роботи не виконує, а лише «перекочується» між джерелом і навантаженням. Тому електропостачальні компанії штрафують підприємства за низький коефіцієнт потужності, а самі підприємства встановлюють паралельно навантаженню батареї конденсаторів --- так звані компенсатори реактивної потужності --- щоб «погасити» індуктивну складову ємнісною і підняти $\cos\varphi_0$ ближче до одиниці.
\end{Attention}