...
той, що вказано на рис.~\ref{tikz:LCR_circuit} для струму $J$.
...
У попередньому розділі ми знайшли комплексну амплітуду напруги на конденсаторі~\eqref{eq:V0_frequency_response}. Тепер нас цікавить не напруга, а струм у колі. Оскільки $J = \dot{q}$ і $V = q/C$, комплексні амплітуди цих величин пов'язані співвідношенням
\begin{equation}\label{eq:J_from_V}
	\hat{J}_0 = i\omega C \hat{V}_0.
\end{equation}
Підставляючи сюди \eqref{eq:V0_frequency_response} і враховуючи, що $\omega_0^2 = 1/(LC)$, отримуємо
\begin{equation}\label{eq:J0_explicit}
	\hat{J}_0 = \frac{i\omega / L}{\omega_0^2 - \omega^2 + 2i\gamma\omega}\emf_0.
\end{equation}
...
\begin{equation}\label{eq:Ohm_complex_derived}
	\tcbhighmath{
		\hat{J}_0 = \frac{\emf_0}{R + i\left(\omega L - \dfrac{1}{\omega C}\right)}.
	}
\end{equation}
...
Запишемо струм у вигляді $J = J_0 e^{i\omega t}$ і помножимо його на імпеданс кожного елемента:
\begin{align}
	U_R &= R J_0 e^{i\omega t}, \label{eq:UR_phase} \\[4pt]
	U_C &= -\frac{i}{\omega C} J_0 e^{i\omega t}
	      = \frac{J_0}{\omega C} e^{i(\omega t - \pi/2)}, \label{eq:UC_phase} \\[4pt]
	U_L &= i\omega L J_0 e^{i\omega t}
	      = \omega L J_0 e^{i(\omega t + \pi/2)}. \label{eq:UL_phase}
\end{align}
...
На рис.~\ref{tikz:vector_diagram_full} показано векторну діаграму для послідовного $LCR$-контуру. Вектор струму $J$ напрямлений уздовж дійсної осі; доданки $RJ$, $i\omega L J$ і $-iJ/(\omega C)$ утворюють прямокутний трикутник, гіпотенуза якого дорівнює вектору ЕРС $\emf$. З діаграми відразу видно, що
\[
	\emf_0^2 = R^2 J_0^2 + \left(\omega L - \frac{1}{\omega C}\right)^2 J_0^2,
\]
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[>=Stealth, line width=0.8pt]
            ...
            \draw[red, ->, thick] (O2) -- (UL_OY) node[left=2pt] {$i\omega L J$};

            \draw[blue, ->, thick] (O2) -- (UC_OY) node[left=2pt] {$-i \frac{J}{\omega C}$};

            \draw[red, ->, thick] (O2) -- (A) node[midway, below=2pt] {$RJ$};

            \draw[green!50!black, ->, thick] (A) -- (C) node[right=2pt] {$i\left(\omega L - \frac{1}{\omega C}\right)J$};
...
потужність, що розсіюється струмом у провідному середовищі, рівна $Q = \emf J$. У
...
\begin{equation}\label{eq:average_power}
	\overline{Q} = \overline{\emf J} = \frac{1}{T}\int\limits_{t}^{t+T} \emf(t_1)J(t_1) \, dt_1.
\end{equation}
...
\begin{equation}\label{eq:product_expansion}
	\begin{aligned}
		\emf(t)J(t) & = \frac{1}{4}\left(\hat{\emf}(t) + \hat{\emf}^*(t)\right)\left(\hat{J}(t) + \hat{J}^*(t)\right) =                                         \\
		            & = \frac{1}{4}\left[\hat{\emf}(t)\hat{J}(t) + \hat{\emf}^*(t)\hat{J}^*(t) + \hat{\emf}(t)\hat{J}^*(t) + \hat{\emf}^*(t)\hat{J}(t)\right] = \\
		            & = \frac{1}{2}\operatorname{Re}\left[\hat{\emf}(t)\hat{J}(t) + \hat{\emf}(t)\hat{J}^*(t)\right].
	\end{aligned}
\end{equation}
...
\end{equation}
(амплітуди $\emf_0$ і $J_0$ тут вважаються дійсними величинами). Це
означає, що доданок $\hat{\emf}(t)\hat{J}(t)$ осцилює з частотою $2\omega$ і його середнє
значення за період $T = 2\pi/\omega$ рівне нулю:
\begin{equation}\label{eq:fast_oscillating_term}
	\overline{\hat{\emf}(t)\hat{J}(t)} = \emf_0 J_0 e^{i\varphi_0} \overline{\exp[2i\omega t]}
	= \emf_0 J_0 e^{i\varphi_0} \frac{1}{T}\int\limits_{t}^{t+T} \exp[2i\omega t_1] \, dt_1 = 0.
\end{equation}
Аналогічно встановлюється, що
\begin{equation}\label{eq:conjugate_term}
	\overline{\hat{\emf}^*(t)\hat{J}^*(t)} = 0.
\end{equation}
В результаті з \eqref{eq:average_power} отримуємо
\begin{equation}\label{eq:average_power_complex}
	\tcbhighmath{
		\overline{Q} = \frac{1}{2}\operatorname{Re}\left[\hat{\emf}(t)\hat{J}^*(t)\right].
	}
\end{equation}
З урахуванням зсуву фаз між струмом і ЕРС звідси знаходимо
\begin{equation}\label{eq:average_power_cos}
	\tcbhighmath{
		\overline{Q} = \frac{1}{2}\emf_0 J_0 \operatorname{Re}\left[e^{-i\varphi_0}\right]
		= \frac{1}{2}\emf_0 J_0 \cos\varphi_0.
	}
\end{equation}
...
\begin{equation}\label{eq:average_power_effective}
	\tcbhighmath{
		\overline{Q} = \emf_{\text{eff}} J_{\text{eff}} \cos\varphi_0.
	}
\end{equation}
Ця формула показує, що чим більше зсув фаз між струмом і напругою, тим менше розсіювана в опорі потужність. У
випадку резонансу, коли $\tg\varphi_0 = 0$, споживана потужність максимальна: $\overline{Q} = \emf_{\text{eff}} J_{\text{eff}}$.

Перепишемо співвідношення \eqref{eq:average_power_complex}, враховуючи закон Ома: $\emf = JZ$, а
...
$LCR$-кон\-ту\-рі (рис.~\ref{tikz:LCR_circuit}) при зміні частоти коливань ЕРС. Згідно з \eqref{eq:Ohm_complex_derived} максимум амплітуди струму $J_0(\omega)$ досягається на частоті $\omega = \omega_0 = 1/\sqrt{LC}$ і становить $J_{0,\max} = \emf_0/R$ (рис.~\ref{tikz:current_resonance}). Це випливає з того, що на частоті $\omega_0$ індуктивний і ємнісний імпеданси точно компенсують один одного:
...
\begin{axis}[
    ...
    declare function={
        ...
        Jmax = E0 / R;                               % Максимальний струм при резонансі
        J0(\w) = E0 / sqrt(R^2 + (\w*L - 1/(\w*C))^2); % Амплітуда струму
    },
    % --- 2. НАЛАШТУВАННЯ ОСЕЙ ---
    axis lines=center,
    xlabel={$\omega$},
    ...
    \draw[dashed, thick] (axis cs:{w0}, 0) -- (axis cs:{w0}, {Jmax});
...
В результаті імпеданс контуру зводиться до активного опору $R$, тож амплітуда струму досягає максимуму $J_{0,\max} = \emf_0/R$. Напруга на конденсаторі при цьому дорівнює:
\begin{equation*}
	V_{C,\max} = \frac{J_{0,\max}}{\omega_0 C} = \frac{\emf_0}{\omega_0 C R}.
\end{equation*}
...
thick]
		...
		node[midway, above] {$J_L$};

		\draw (L2) to[C, l=$C$] (R2);
		\draw[<-, >=latex, thick] ($(L2)!0.82!(R2) + (0, 0.35)$) -- ($(L2)!0.98!(R2) + (0, 0.35)$)
		node[midway, above] {$J_C$};

		\draw (L1) to[R, l=$R$] (R1);
		\draw[<-, >=latex, thick] ($(L1)!0.82!(R1) + (0, 0.35)$) -- ($(L1)!0.98!(R1) + (0, 0.35)$)
		node[midway, above] {$J_R$};

		\draw (L0) to[isourcesin, sources/symbol/rotate=0, l_=$J(t)$] (R0);
	...
	\end{circuitikz}
	\caption{Паралельний $LCR$-контур, що включає джерело заданого змінного струму $J(t) = J_0 \cos\omega t$.}
...
\begin{equation}\label{eq:JR_amplitude}
	J_{R_0} = \frac{J_0}{\sqrt{1 + \left(\dfrac{R}{\omega L} - \omega C R\right)^2}}
	= \frac{J_0}{\sqrt{1 + \dfrac{4\gamma^2}{\omega^2}\left(1 - \dfrac{\omega^2}{\omega_0^2}\right)^2}}.
\end{equation}
Звідси видно, що $J_{R_0}$ має максимум при $\omega = \omega_0$ і становить $(J_{R0})_{\max} = J_0$. Це означає, що на резонансній частоті весь струм від джерела тече тільки через активний опір. Справа в тому, що опір блока з паралельно з'єднаних конденсатора й котушки на цій частоті обертається в нескінченність, і струм через нього не тече.
...
\begin{enumerate}
	...
	\item Для будь-якого замкнутого контуру в квазістаціонарному електричному колі виконується рівність
	      \begin{equation}\label{eq:Kirchhoff_voltage}
		      \sum_k J_k Z_k = \sum_i \emf_i,
	      \end{equation}
	      де $\emf_i$ --- ЕРС, що входять у вибраний контур, $J_k$ --- струми через елементи контуру, що мають імпеданси $Z_k$.
	      ...
	      \begin{SCfigure}[1.75][h!]
		      ...
		      \begin{tikzpicture}[>=Stealth, line width=1.2pt]
			      ...
			      \draw[->, red] (-1.5, 1.3) -- (O) node[above left=1pt, pos=0] {$J_1$};
			      \draw[->, blue] (1.5, 1.3) -- (O) node[above right=1pt, pos=0] {$J_2$};
			      \draw[<-, green!50!black] (0, -2.0) -- (O) node[midway, right=3pt] {$J_3$};
...
\begin{center}%[h!]
	...
	\begin{circuitikz}[european resistors, scale=1.4, circuitikz/bipoles/length=1.8cm, thick]
		...
		% Стрілка загального струму J (від джерела до вузла E)
		\draw[->, >=latex, thick] ($(D)!0.7!(E) + (0,0.45)$) -- ($(D)!0.95!(E) + (0,0.45)$)
		node[midway, above] {$J$};

		% Стрілка струму J_R через резистор (від E до B)
		\draw[->, >=latex, thick] ($(E)!0.3!(B) + (0.35,0)$) -- ($(E)!0.7!(B) + (0.35,0)$)
		node[midway, right] {$J_R$};

		% Стрілка струму J_C через конденсатор (від E до F)
...
Застосуємо \emphx{перше правило Кірхгофа} до вузла $E$: сума струмів, що виходять із вузла, дорівнює нулю. Струм $J$ приходить до вузла $E$, а струми $J_R$ і $J_C$ виходять із нього, тому
...
Маємо систему трьох рівнянь \eqref{eq:example_KCL}--\eqref{eq:example_KVL_II} із трьома невідомими $J$, $J_R$, $J_C$. Розв'яжемо її. З \eqref{eq:example_KVL_I} і \eqref{eq:example_KVL_II} виразимо $J_R$ і $J_C$ через $J$:
\begin{equation}\label{eq:example_JR_JC}
	J_R = \frac{\emf - Z_L J}{R}, \qquad
	J_C = \frac{\emf - Z_L J}{Z_C}.
\end{equation}
Підставимо ці вирази в \eqref{eq:example_KCL}:
\begin{equation}\label{eq:example_J_equation}
	J = \frac{\emf - Z_L J}{R} + \frac{\emf - Z_L J}{Z_C}.
\end{equation}
Розв'язуючи відносно $J$, отримуємо
\begin{equation}\label{eq:example_current}
	J = \frac{\emf}{Z_L + \dfrac{1}{\dfrac{1}{R} + \dfrac{1}{Z_C}}}.
\end{equation}
...
\begin{equation}\label{eq:example_current_explicit}
	J = \frac{\emf_0 e^{i\omega t}}{i\omega L + \dfrac{R}{1 + i\omega RC}}
...
\begin{equation}\label{eq:example_branch_currents}
	J_R = \frac{U_{RC}}{R} = \frac{\emf_0 e^{i\omega t}}{R(1 - \omega^2 LC) + i\omega L},
\end{equation}
\begin{equation}\label{eq:example_branch_currents_C}
	J_C = i\omega C\,U_{RC} = \frac{i\omega RC\,\emf_0 e^{i\omega t}}{R(1 - \omega^2 LC) + i\omega L}.
\end{equation}

Перевіримо виконання першого правила Кірхгофа \eqref{eq:example_KCL}:
\begin{equation*}
	J_R + J_C = \frac{\emf_0 e^{i\omega t}(1 + i\omega RC)}{R(1 - \omega^2 LC) + i\omega L} = J,
\end{equation*}
...
\paragraph*{Фільтр низьких частот (ФНЧ).}
...
\paragraph*{Фільтр високих частот (ФВЧ).}
...
\begin{quizlist}
	...
	\item Виведіть рівняння $L\dot J + JR + q/C = \emf$ для послідовного $LCR$-контуру (рис.~\ref{tikz:LCR_circuit}). Яку роль відіграє теорема про циркуляцію електричного поля і як вона враховує ЕРС індукції в котушці?
	...
	\item Виведіть рівняння енергетичного балансу $\frac{d}{dt}\left(\frac{LJ^2}{2}+\frac{q^2}{2C}\right) = -RJ^2$. Що означають його доданки?
	\item Як перетворюється енергія в контурі без втрат? Чому енергії $W_L$ та $W_C$ коливаються з однаковою амплітудою в протифазі, а їхня сума залишається сталою? Який зсув фаз між $q(t)$ і $J(t)$?
	...
	\item Виведіть формулу для середньої потужності змінного струму $\overline Q = \frac12\emf_0J_0\cos\varphi_0$. Що таке ефективні значення струму й напруги і чому потужність залежить від зсуву фаз?
